...
class TestMeerKATMonitoringAndArchiving(AqfTestCase):
    ...
    def katstore_check_sensor_samples(self, sensors_list, start_sec,
        ...
        for idx,sens in enumerate(sensors_list):
            ...
            if not it_passed:
                ...
                # Test historical_sensor_timeranges
                # response = self.cam.katstore_query.req.historical_sensor_timeranges(
                #     sens_name, start_sec, end_sec,
                #     timeout=self.DEFAULT_KATCP_TIMEOUT)
                # lines = '\n'.join([msg.arguments[0]
                #                    for msg in response.messages[1:]])
                ...
                if not product_found:
                    # Possibly more than one ccsv file reference returned
                    # it_passed = len(lines.splitlines()) >= 1
                    # if not it_passed:
                    #     Aqf.failed("Expected sensor returned no "
                    #            "historical sensor timeranges: %s" % (sens_name))
                    #     errors += 1
                    # else:
                    #     Aqf.passed("Expected sensor returned expected "
                    #            "historical sensor timeranges: %s" % (sens_name))
                    ...
                    # Skip checking the dynamically created sensors for now
                    Aqf.passed("Skip checking of dynamically created sensors "
    ...
    def _get_sample_stats(self, str_samples, start_sec, end_sec):
        ...
        if samples:
            ...
            Aqf.progress ("No samples returned for period %s (%s) to %s (%s)"
                % (Timestamp(start_sec), start_sec, Timestamp(end_sec), end_sec))
            ...
            if strategy == 'event-rate':
                ...
                if actual_sample_count in range(expected_min, expected_max + 1):
                    ...
                    if actual_sample_count > 0:
                        actual_sample_start = samples[0]
                        actual_sample_end = samples[-1]
                        Aqf.failed("Sensor %s: %r Actual samples %d not in expected "
                                   "sample range (%d, %d) "
                                   "actual sample start and end (%s, %s) "
                                   "(start_sec=%s, end_sec=%s, strategy='period' %s)" %
                                   (sens_name, result, actual_sample_count,
                                    expected_min, expected_max,
                                    str(actual_sample_start), str(actual_sample_end),
                                    start_sec, end_sec, strategy_params))
                    else:
                        Aqf.failed("Sensor %s: %r Actual samples %d not in expected "
                                   "sample range (%d, %d) "
    ...
    def _test_monitor_archive_a_device(self, proxy, device):
        ...
        ##self.cam.katstore_query.req.refresh_sensor_list()

        Aqf.step("Extract sensor list for proxy %s device %s" %
                 (proxy, device))


        # Extract sensors for proxy_device_* from cam.sensors
    ...
    def test_config_of_monitor_freq_startup(self):
        """Test the startup configuration of the monitoring frequency of sensors.
        ...
        run_time = time.time() - start_sec
        if run_time >= self.MIN_RUN_PERIOD_SEC:
            Aqf.step("System has been running since %s for %s => "
                     "Fetching samples for period=%s from (system_start+%s) to (now-%s)" %
                     (system_start_time, Duration(run_time), Duration(end_sec-start_sec),
            ...
            # Test each of the monitored sensors
            failed_sens = ""
            total_sens = 0
            for mss in monitored_sensor_strategies:
                ...
                err = self._check_historical_sensor_data(query_sens_name, start_sec,
                    end_sec, strategy, strategy_params, thorough=True)
                if err:
                    failed_sens = ",".join([failed_sens,query_sens_name])
                    errors += err
                total_sens += 1

            Aqf.equals(errors, 0, "Number of errors during test: %d/%d. Failed: %s" % (errors, total_sens, failed_sens))
        else:
            Aqf.skipped("System has not been running long enough to "
                        "save cached monitoring data to disk (%s < %s) - "
    ...
    def TODO_xtest_config_of_monitor_freq_runtime(self):
        ...
        test_sensors = random.sample(self.cam.sensor.keys(),
                                     NUM_SENSORS_TO_TEST)
    ...
    @aqf_vr("VR.CM.DEMO.M.26")
    @system("all")
    def test_inspect_monitor_strategies_config(self):
        """Inspect CAM allow configuration of monitoring frequency"""

        Aqf.step("For the current system under test, open up the appropriate monitor "
                 "strategies configuration file located at "
                 "*/var/kat/katconfig/static/strategies* on the head node.")
        Aqf.checkbox("Verify that the configuration file is human readable and easy to "
                     "edit.")
        Aqf.checkbox("Verify that each monitoring point and its strategy can be uniquely "
                     "identified from the sensor name and the component/proxy it belongs "
                     "to.")
        Aqf.checkbox("Verify that for each monitoring point the strategy and strategy "
