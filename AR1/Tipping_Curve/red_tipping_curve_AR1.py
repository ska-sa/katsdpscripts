...
# MK_L_Tspill_AsBuilt_atm_mask.dat includes ground & atmospheric radiation
# pickup, the cosmic microwave background and an average Galactic noise of 20 K
# at 408 MHz, but the main beam and first sidelobe is zeroed out in the pattern
# integral. The ground is assumed to be in thermal equilibrium at 300 K with
# reflection approximated as the average of that of H & V. The atmosphere is
# modelled with the sky brightness model of Cortes-Medellin in SKA Memo 95, employing nominal values for atmospheric temperature, pressure and water vapour density which is believed to be representative of the Karoo environment except for winter time.
class Spill_Temp:
...
##Output and Report
    import os
    from matplotlib.backends.backend_pdf import PdfPages
    nice_filename =  os.path.splitext(os.path.basename(filename))[0]+ '_' +ant.name+'_tipping_curve'
