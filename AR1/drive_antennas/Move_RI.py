...
def track(ants, target, ridx_position='l', duration=10, dry_run=False):
    ...
    if not dry_run:
        for ant_x in ants:
            if ant_x.wait('lock', True, timeout=300):
                ...
                locks += 1
            user_logger.info("%s out of %s antennas on target" %
                             (locks, len(ants)))
    else:
        locks = len(ants)
    if len(ants) == locks:
        user_logger.info("Target reached. Wait for %d seconds "
                         "before starting indexer cycle" % duration)
        if not dry_run:
            time.sleep(duration)
    else:
    ...
    if not dry_run:
        ...
        time.sleep(2)
        result = wait_until_sensor_equals(5.0, 'm036_mode', 'STOP')
        if result[0] == False:
    ...
    if result[0] == False:
        ...
        ridx_position_raw = kat.sensors.get('m036_ap_indexer_position_raw', None).get_value()
    ...
    if not dry_run:
        for pos in ridx_sequence:
            # TODO: support selectable AP, not just m036
            ridx_last_position = kat.sensors.get('m036_ap_indexer_position', None).get_value()
            ...
            if result[0] == False:
                #  TODO: support selectable AP, not just m036
                ridx_position_raw = kat.sensors.get('m036_ap_indexer_position_raw', None).get_value()
                ridx_brakes_released = kat.sensors.get('m036_ap_ridx_brakes_released', None).get_value()
            ...
            # TODO: support selectable AP, not just m036
            ridx_current_position = kat.sensors.get('m036_ap_indexer_position', None).get_value()
...
with verify_and_connect(opts) as kat:
    ...
    for taz, tel, band in targetlist:
        for i in range(int(opts.num_repeat)):
            ...
            track(kat.ants, target, ridx_position=band,
