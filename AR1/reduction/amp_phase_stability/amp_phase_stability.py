...
def text_calc(windowtime,timestamps,peak,std,dtrend_std,phase=True,title=""):
    sub_title = "Amplitude"
    unit_str = ""
    unit = 1.0
    if phase :
...
def calc_stats(timestamps, gain, pol='no polarizarion', windowtime=30, minsamples=30,title="",noplot=True):
    ...
    pltobj = []
    #Phase 
    #Series.rolling(center=False,min_periods=30,window=30).apply(args=<tuple>,func=<function>,kwargs=<dict>)
    ...
    if valid_target :
        for pol in ('h','v'):
            ...
            for scan in h5.scans():
                ...
                if scan[2].name not in bandpass:
