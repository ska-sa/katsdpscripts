...
if __name__=="__main__":
    ...
    parser.add_option("-c", "--chanOff", type="float", default=-0.208984, 
    help="Channel bandwidth override (def = %default MHz).")
    parser.add_option("-f", "--fch1", type="float", default=1444.4, 
    help="Top-of-band channel centre frequency override (def = %default MHz).")
    ...
    #################################################################
    # Check decimation factor and write out sigproc header to fbank.
    #################################################################
    if ( opts.ndec > 1 ): 
    ...
    hdr_cmd += '-nifs 1 -tsamp %.10f -fch1 %.6f -fo %.6f -nchans %i %s' %(tsamp,opts.fch1,opts.chanOff,nchan,outfile)
    os.system(hdr_cmd)
