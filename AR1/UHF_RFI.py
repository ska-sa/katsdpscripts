...
parser = standard_script_options(usage="%prog [options] <'target/catalogue'> [<'target/catalogue'> ...]",
                                 description='Track one or more sources for a specified time. This script  '
                                             'triggers the signal generator to fluctuate the power to simulate'
                                             ' some simple rfi. Note also some **required** options below.')
# Add experiment-specific options
parser.add_option('-t', '--track-duration', type='float', default=5.0,
                  help='Length of time to track each source, in seconds this is also the rfi period (default=%default)')
parser.add_option('-m', '--max-duration', type='float', default=120,
...
parser.add_option('--siggen-power',  default='-20.0,-100.0',
                  help='Signal Generator power values in dBm (default=%default)')
...
if  np.any(np.array(opts.siggen_power.split(',')).astype(float) > -20.):
    raise ValueError("Please specify a Signal Generator power less than -20 dBm")


siggen_ip = opts.siggen_ip
siggen_port = int(opts.siggen_port)
siggen_freq = np.float(opts.siggen_freq)
siggen_power_list = np.array(opts.siggen_power.split(',')).astype(float)
...
## Using SCPI class for comms to signal generator for CW input signal
            ...
            while keep_going:
                ...
                for target in observation_sources.iterfilter(el_limit_deg=opts.horizon):
                    if not kat.dry_run or opts.force_siggen :
                        sig.setPower(siggen_power[np.mod(iteration_count,len(siggen_power_list))])
                        user_logger.info("Signal Generator Power is set to %f dBm"%(sig.getPower()))
                        siggen_power=sig.getPower()
                        # time.sleep(2) not need for this script
                        
                    #mod(iteration_count,len(siggen_power_list))
