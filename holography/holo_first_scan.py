...
#   az=-0.02;el=-0.1; kat.ant1.req.offset_fixed(az,el,'stereographic');
# Now iterate on az, el to maximize the magnitude
...
parser.set_defaults(description='Holography script version 1')
...
with verify_and_connect(opts) as kat:
    ...
    with CaptureSession(kat, **vars(opts)) as session:
        ...
        if az is not None:
            user_logger.info("Adding azimuth offset of: %g" % az)
            t_az += az
        if el is not None:
            user_logger.info("Adding elevation offset of: %g" % el)
            t_el += el
        ...
        for x in np.arange(-nrsteps,nrsteps,1):
            ...
            kat.ant1.wait('scan_status', 'after', 25*degrees+300)
