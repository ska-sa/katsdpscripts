...
def deploy():
    ...
    for name in PUBLIC_SKA_GIT: install_git_package(name)

    # install private ska-sa git packages
    for name in PRIVATE_SKA_GIT: install_git_package(name,user=GIT_USER,password=GIT_PASSWORD)

    # create git package directory
    run('mkdir %s' % (SVN_DIR,))

    # install svn packages
    for name in SVN_PKGS: install_svn_package(name,SVN_USER,SVN_PASSWORD)

    # install katconfig files
    print ' ---- Install katconfig ---- \n'
