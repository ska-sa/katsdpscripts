...
def fit_model(filename, use_stats=True, min_rms=np.sqrt(2)*60.*1e-12, outfilebase='', pmfilename=None, interactive=True, enabled_params=[]):
    """
        Fits a pointing model based on the data in the specified file.
        
        @param filename: CSV from analyze_point_source_scans.py
        @param use_stats: Ignore uncertainties of data points during fitting (default False)
        @param min_rms: Minimum uncertainty of data points, expressed as the sky RMS in arcminutes (default sqrt(2)*60*1e-12*60 i.e. 1e-12 deg per axis)
                    - the least-squares solver does not like zero uncertainty!
        @param outfilebase: In interactive mode this is the base name of output files (default '')
        @param pmfilename: Name of optional file containing old pointing model, overriding the one in CSV file (default None)
        @param interactive: True to launch an interactive GUI instead of just fitting based on the function's arguments (default True)
        @param enabled_params: sequence of ints or bools for model parameters used for fitting, instead of what's in the existing model (default None).
                    Integers start at 1 and correspond to the P-number of the model terms. This list can be modified in interactive mode.
        @return: katpoint.PointingModel   
    """
    ...
    if pmfilename:
        try:
            old_model = katpoint.PointingModel(open(pmfilename).readline())
            logger.debug("Loaded %d-parameter pointing model from '%s'" % (len(old_model), pmfilename))
        except IOError:
            logger.warning("Could not load old pointing model from '%s'" % (pmfilename,))
    ...
    # enabled_params may also be explicitly forced
    if (enabled_params is None or (len(enabled_params)==0)):
    ...
    def update(fig=None):
        """Fit new pointing model and also update plots if 'fig' is not None."""
        ...
        if (np.count_nonzero(keep) > 0): # Avoid exceptions when there are no data points to fit to 
            ...
            print("WARNING: stale model, can't update if all points have been discarded!")
    ...
    # In non-interactive mode simply update the model & return it
    if not interactive:
        update(fig=None)
        return new_model
    # Or for interactive mode, set-up the GUI & return the updated model when done
    ...
    def save_callback(event):
        ...
        outfile = open(outfilebase + '.csv', 'w')
        # The original pointing model description string was comma-separated
        outfile.write(new_model.description.replace(" ", ", "))
        outfile.close()
        logger.debug("Saved %d-parameter pointing model to '%s'" % (len(new_model), outfilebase + '.csv'))
        ...
        outfile2 = open(outfilebase + '_data.csv', 'w')
...
if __name__ == "__main__":
    ...
    # Fit the model interactively
    fit_model(filename, opts.use_stats, opts.min_rms, opts.outfilebase, opts.pmfilename, interactive=True)
