...
def haversine(az1,el1, az2,el2):
    ...
    r = np.degrees(1) #6371 # Radius of earth in kilometers. Use 3956 for miles
    return c * r
...
def antenna_stats(h5,ants='',slew_from_angles=(30,7)):
    ...
    for ant in ants : 
        request_az = unpickle(h5,'TelescopeState/%s_pos_request_scan_azim'%(ant,))
        request_el = unpickle(h5,'TelescopeState/%s_pos_request_scan_elev'%(ant,))
        az = unpickle(h5,'TelescopeState/%s_pos_actual_scan_azim'%(ant,))
        el = unpickle(h5,'TelescopeState/%s_pos_actual_scan_elev'%(ant,))
        activity,activity_lookup = unpickle_cat(h5,'TelescopeState/%s_activity'%(ant,))

        ts_el = pd.Series(el[:,1], index=(el[:,0]*1e9).astype('datetime64[ns]'))
        ts_request_el = pd.Series(request_el[:,1], index=(request_el[:,0]*1e9).astype('datetime64[ns]')).reindex_like(ts_el, method='ffill')
        ts_az = pd.Series(az[:,1], index=(az[:,0]*1e9).astype('datetime64[ns]'))
        ts_request_az = pd.Series(request_az[:,1], index=(request_az[:,0]*1e9).astype('datetime64[ns]')).reindex_like(ts_az, method='ffill')
        ts_activity = pd.Series(activity[:,1], index=(activity[:,0]*1e9).astype('datetime64[ns]'))
        #delta_el = ts_request_el-ts_el
        #delta_az = ts_request_az-ts_az    
        delta_sky  = haversine(ts_request_az.reindex_like(ts_az,method='nearest'),ts_request_el.reindex_like(ts_az,method='nearest'), ts_az.reindex_like(ts_az,method='nearest'),ts_el.reindex_like(ts_az,method='nearest'))

        locked  = (delta_sky.rolling(pd.Timedelta(seconds=2) , min_periods=1, ).max() < 0.01 ) 
        locked.index = locked.index + pd.Timedelta(seconds=2) # move window foward edge
        locked = locked.reindex_like(ts_az,method='nearest')  & (delta_sky < 0.01)

        if not activity_lookup.has_key('track') : 
            activity_lookup['track'] = -1 #  not found
        tracking = (ts_activity.reindex_like(ts_az,method='ffill') == activity_lookup['track']) & (delta_sky < 0.01)
        if not activity_lookup.has_key('scan') : 
            activity_lookup['scan'] = -1 #  not found
        scaning = (ts_activity.reindex_like(ts_az,method='ffill') == activity_lookup['scan']) & (delta_sky < 0.01)
                ...
                if x :
                    for j in range(60+int(2*angle_param)): # time to get lock
                        if delta_sky[i+j] < 0.01 and locked[i+j] :
                            seconds =  (delta_sky.iloc[[i+j] ].index - delta_sky.iloc[[i] ].index ).total_seconds()[0]
