...
def find_jumps(timestamps, power, std_power, margin_factor, jump_significance, max_onoff_segment_duration, **kwargs):
    ...
    rise = np.where((power > previous_upper) &
                    (power < next_upper))[0].tolist()
    ...
    # Then pick midpoint of jump (or end of jump if no clear midpoint), and add it to candidate jumps
    drop = np.where((power < previous_lower) &
                    (power > next_lower))[0].tolist()
    ...
    for jump in jumps:
        ...
        segment_range = np.abs(
            timestamps - timestamps[jump]) <= max_onoff_segment_duration
        ...
        after = max(len(segment_range) - 1 -
                    np.where(segment_range[::-1])[0][0], jump + 1)
        before = max(
...
for ant in data.ants:
    ...
    for diode_name in ('pin', 'coupler'):
        ...
        try:
            sensor = data.sensor.get('Antennas/%s/nd_%s' %
        ...
        for scan_index, state, target in data.scans():
            ...
            power_dof = dof / \
                2 if (hh_index is not None and vv_index is not None) else dof
            ...
            firings_in_scan = (nd_timestamps > dbe_timestamps[1]) & (
                nd_timestamps < dbe_timestamps[-1])
