...
for freq in np.array(opts.freq.split(',')).astype(float).tolist():
    ...
    if True : # R.RC.P.2 & R.RC.P.3
        ...
        dx = (10*np.log10(np.abs(innerbeam.Dx[mask_3dB])/scalefunc**2)>-20).sum()
        dy = (10*np.log10(np.abs(innerbeam.Dy[mask_3dB])/scalefunc**2)>-20).sum()
        ...
        dx = (10*np.log10(np.abs(innerbeam.Dx[mask_1dB])/scalefunc)>-26).sum()
        dy = (10*np.log10(np.abs(innerbeam.Dy[mask_1dB])/scalefunc)>-26).sum()
        ...
        fig = plt.figure()
        plt.imshow(10*np.log10(np.abs(innerbeam.Dx[0,...]))*mask_3dB[0,...],extent=innerextents)
        plt.colorbar()
        ...
        plt.close(fig)
        fig = plt.figure()
        plt.imshow(10*np.log10(np.abs(innerbeam.Dy[0,...]))*mask_3dB[0,...],extent=innerextents)
        plt.colorbar()
        plt.xlabel('Degrees')
        plt.ylabel('Degrees')
