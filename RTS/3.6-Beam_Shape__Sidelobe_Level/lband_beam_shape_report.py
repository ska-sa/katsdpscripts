...
def makereport(dataset,beams,beamemss):
    ...
    with PdfPages(os.path.splitext(os.path.basename(filename))[0]+reportfilename) as pdf:
        ...
        for ifreq,thefreq in enumerate(freqs):
            ...
            plt.plot(margin,bm[gridsize/2,:],'r')
            plt.plot(margin*np.sqrt(2),[bm[i,i] for i in range(len(margin))],'g')
            plt.plot(margin,bm[:,gridsize/2],'b')
            plt.plot(margin*np.sqrt(2),[bm[-1-i,i] for i in range(len(margin))],'c')
            plt.axhline(-23,color='k',linestyle='--')
            plt.xlim([-extent/2,extent/2])
            plt.ylim([-50,0])
            if (ifreq<len(freqs)-1):
                plt.xlabel('')
            if (ifreq==0):
            ...
            plt.plot(margin,bm[:,gridsize/2],'b')
            plt.plot(margin*np.sqrt(2),[bm[-1-i,i] for i in range(len(margin))],'c')
            plt.plot(np.linspace(beamemss[ifreq].margin[0],beamemss[ifreq].margin[-1],beamemss[ifreq].gridsize),-20-6.0*(bc>-1.0),'k--')
            plt.xlim([-extent/2,extent/2])
            plt.ylim([-50,-20])
            if (ifreq<len(freqs)-1):
                plt.xlabel('')
        ...
        for ifreq,thefreq in enumerate(freqs):
            ...
            plt.subplot(len(freqs),4,1+ifreq*len(freqs))
            plt.imshow(mdGx,extent=[-ext,ext,-ext,ext],clim=[0,0.05])
            plt.xlim([-1.5,1.5])
            plt.ylim([-1.5,1.5])
            plt.colorbar()
            if (ifreq<len(freqs)-1):
                plt.xlabel('')
            plt.title('Gx (max %.2f%%)'%(mdGxmax[ifreq]*100.0))
            plt.subplot(len(freqs),4,2+ifreq*len(freqs))
            plt.imshow(dGx,extent=[-ext,ext,-ext,ext],clim=[0,0.05])
            plt.xlim([-1.5,1.5])
            plt.ylim([-1.5,1.5])
            plt.colorbar()
            if (ifreq<len(freqs)-1):
                plt.xlabel('')
            plt.ylabel('')
            plt.title('Gx (stddev %.2f%%)'%(dGxstdev[ifreq]*100.0))
            plt.subplot(len(freqs),4,3+ifreq*len(freqs))
            plt.imshow(mdGy,extent=[-ext,ext,-ext,ext],clim=[0,0.05])
            plt.xlim([-1.5,1.5])
            plt.ylim([-1.5,1.5])
            plt.colorbar()
            if (ifreq<len(freqs)-1):
                plt.xlabel('')
            plt.ylabel('')
            plt.title('Gy (max %.2f%%)'%(mdGymax[ifreq]*100.0))
            plt.subplot(len(freqs),4,4+ifreq*len(freqs))
            plt.imshow(dGy,extent=[-ext,ext,-ext,ext],clim=[0,0.05])
            plt.xlim([-1.5,1.5])
            plt.ylim([-1.5,1.5])
            plt.colorbar()
            if (ifreq<len(freqs)-1):
                plt.xlabel('')
            plt.ylabel('')
            plt.title('Gy (stddev %.2f%%)'%(dGystdev[ifreq]*100.0))
        plt.suptitle('Error beam and polynomial fit within -12dB region\n%dMHz %dMHz %dMHz %dMHz\nMax error: %.2f%%'%(freqs[0],freqs[1],freqs[2],freqs[3],100.0*np.max([np.max(mdGxmax),np.max(mdGymax)])))
        pdf.savefig()
    ...
    with PdfPages(os.path.splitext(os.path.basename(filename))[0]+debugreportfilename) as debugpdf:
        ...
        for iant in range(len(dataset.radialscan_allantenna)):
            for jant in range(iant+1,len(dataset.radialscan_allantenna)):
                ...
                plt.figure(ifig,figsize=(16, 8), dpi=100)
                ifig+=1
                plt.clf()
                plt.subplot(4,1,1)
                plt.imshow((np.angle(dataset.h5.vis[:,:,cpindices[0]].squeeze())))
                plt.ylabel('time [sample]')
                plt.title('XX')
                plt.subplot(4,1,2)
                plt.imshow((np.angle(dataset.h5.vis[:,:,cpindices[1]].squeeze())))
                plt.ylabel('time [sample]')
                plt.title('XY')
                plt.subplot(4,1,3)
                plt.imshow((np.angle(dataset.h5.vis[:,:,cpindices[2]].squeeze())))
                plt.ylabel('time [sample]')
                plt.title('YX')
                plt.subplot(4,1,4)
                plt.imshow((np.angle(dataset.h5.vis[:,:,cpindices[3]].squeeze())))
                plt.ylabel('time [sample]')
                plt.xlabel('channel')
                plt.title('YY')
                plt.suptitle(dataset.filename+": "+dataset.radialscan_allantenna[iant]+"-"+dataset.radialscan_allantenna[jant]+" "+dataset.target.name+"\nOn-target phase waterfall")
                debugpdf.savefig()
