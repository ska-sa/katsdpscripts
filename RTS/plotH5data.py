#!/usr/bin/env python
# plotH5data.py , last mod 28/01/2016 [NJY]
...
def plot_data(fig,corrProds,x,y,labels,autoCorr,logSwitch,plotIndex):
...
parser.add_option("-f", "--file", default=None, type='string', help="Input file override (default = latest).")
...
fig = plot_data(fig,h5.corr_products,tvals,allTS,
    ['Time (s)','Power (A.U.)'],opts.autoOnly,1,1)
fig = plot_data(fig,h5.corr_products,freqs/1e6,allBPs,
    ['Frequency (MHz)','Power (A.U.)'],opts.autoOnly,1,2)
fig = plot_data(fig,h5.corr_products,all_rxFreqs/1e6,all_rxTsys,
    ['Frequency (MHz)',r'$T_{\mathrm{rx}}$ (K)'],opts.autoOnly,0,3)
fig = plot_data(fig,h5.corr_products,all_noiseFreqs/1e6,all_noiseTsys,
    ['Frequency (MHz)',r'$T_{\mathrm{nd}}$ (K)'],opts.autoOnly,0,4)
