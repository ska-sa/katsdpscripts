...
parser.add_option('--noise-time', type='int', default=600,
                  help="Switch noise diode on (contionous firing) on all antennas for "
                       "first %default seconds.")
...
with verify_and_connect(opts) as kat:
    ...
    with start_session(kat, **vars(opts)) as session:
        ...
        # A quick hack for Tau A dirft scan to work. It should be addressed in a better way tough.
        timestamp = time.time() + 1
        user_logger.info('Set all noise diodes on with timestamp %d (%s)',
                         int(timestamp), time.ctime(timestamp))
        kat.ants.req.dig_noise_source(timestamp, 1)  # Switch noise diode on, on all antennas.
        time.sleep(opts.noise_time)
        kat.ants.req.dig_noise_source('now', 0)  # Switch noise diode off, on all antennas.
        user_logger.info('Set all noise diodes off with timestamp %d (%s)',
                         int(time.time()), time.ctime(time.time()))

        time.sleep(2)  # Allow for 2 seconds to 
