...
def generatepattern(totextent=10,tottime=1800,tracktime=5,slowtime=6,sampletime=1,scanspeed=0.15,slewspeed=-1,twistfactor=1.,trackinterval=1,slowextent=0,slowfactor=2,kind='spiral',calculate_derivative=False):
    ...
    if kind=='spiral':
        ...
        compositey=[np.array(flaty)]
        compositeslew=[np.array(flatslew)]
...
def gen_scan(lasttime,target,az_arm,el_arm,timeperstep,high_elevation_slowdown_factor=1.0,clip_safety_margin=1.0,min_elevation=15.,max_elevation=90.):
    if opts.kind=='azimuth_scan':#assumed that target is azel, GSM tower
        ...
        num_scan_points=int((180)/(opts.scanspeed*opts.sampletime))#this is to test scan extent only
        ...
        targetaz,targetel=targetaz_rad*180./np.pi,targetel_rad*180./np.pi
        if np.mean(targetaz)<45:#45 deg is center of -180 to +270 deg azimuth range
            beamaz=np.r_[np.zeros(num_track_points+num_loop_points+num_el_slew_points+num_loop_points),np.linspace(0,180,num_scan_points)]#beamaz=scanaz-targetaz
        else:
            beamaz=np.r_[np.zeros(num_track_points+num_loop_points+num_el_slew_points+num_loop_points),np.linspace(0,-180,num_scan_points)]#beamaz=scanaz-targetaz
        scanel=drift_el+targetel*np.cos(beamaz*np.pi/180.0)
        scanaz=beamaz+targetaz
...
if __name__=="__main__":
    ...
    parser.add_option('--slowfactor', type='float', default=0,
                      help='Number of times scanspeed is slowed down within slowextent (default=%default)')        
    ...
    if testmode:
        ...
        if opts.kind=='horizon_scan_ext':
            ...
            ensure_cat={
            'azel target':'azel target, azel, 10:00:00.00, 28:00:00'
            }
            ...
            with start_session(kat, **vars(opts)) as session:
                ...
                while cycle<opts.num_cycles or opts.num_cycles<0:
                    ...
                    for igroup in grouprange:
                        ...
                        lasttime = time.time()
                        user_logger.info("Scanpattern alignment timestamp: %s"%(lasttime))
