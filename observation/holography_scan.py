...
with verify_and_connect(opts) as kat:
    ...
    if len(targets.targets) == 0:
        ...
        with start_session(kat, **vars(opts)) as session:
            ...
            user_logger.info("Using all antennas: %s" % (' '.join([ant.name for ant in session.ants.devs]),))
            ...
            for target in targets:
                ...
                for scan_index, (start, end) in enumerate(zip(scan_starts, scan_ends)):
                    ...
                    user_logger.info("Using scan antennas: %s" % (' '.join([ant.name for ant in session.ants.devs]),))
