#!/usr/bin/python
# Perform raster holography scans on specified target(s) where there is a significant difference between scanning region and target
        ...
        for cycle in range(opts.num_cycles):
            for target in targets.iterfilter(el_limit_deg=opts.horizon+(opts.scan_extent/2.0)):
                ...
                if (opts.elevation_scan):
                    for scan_index, azangle in enumerate(np.linspace(-opts.scan_extent / 2., opts.scan_extent / 2.0, opts.num_scans)):
                        ...
                        if (scan_index%opts.num_grouped_scans==opts.num_grouped_scans-1):
                            user_logger.info("Slewing scan antennas to target: %s" % (' '.join([ant.name for ant in session.ants]),))
