...
def moveRI(ants, ridx_pos, dry_run=False):
    ...
    if not dry_run:
	    ...
	    not_on_pos = [ant.name for ant in ants if ant.sensor.ap_indexer_position.get_value() != ridx_pos]
            ...
            # allow brekes to engage and put antennas to stop
            time.sleep(3)
	    ants.req.mode('STOP')
            time.sleep(2)

def pointAnts(ants, dry_run=False):
...
with verify_and_connect(opts) as kat:
    ...
    # point antennas
    # could be redundant code: kat.ants.set_sampling_strategy('lock', 'event')
    if opts.move_indexer:
        moveRI(kat.ants, opts.ridx_pos, dry_run=kat.dry_run)
    pointAnts(kat.ants, dry_run=kat.dry_run)
