...
def subarray_activity(kat):
    ...
    if not kat.subarray_1.sensor.state.get_value() == 'active':
        ...
        if kat.subarray_1.sensor.script_status.get_value() == 'busy':
            ...
            obs_description =  kat.subarray_1.sensor.observation_script_description.get_value()
            sub_details     = ' current obs:\t {}{}'.format(' ' * 20, obs_description,)
            ...
            if obs_duration:
                ...
                sub_details+= '\n duration: \t {}{} - {}\n'.format(
                    ' ' * 20,
                    time.strftime( '%Y-%m-%d %H:%M:%S', time.gmtime( float(obs_start_time) )),
...
with verify_and_connect(opts) as kat:
    ...
    ant_inactive = [
        ant for ant in kat.ants if
        ant.name in kat.katpool.sensor.resources_in_maintenance.get_value() or
        ant.name in kat.subarray_7.sensor.pool_resources.get_value()
    ]
    ...
    print( "\n {}-band MeerKAT status at:           {}".format(opts.receiver_band.upper(), now) )
    ...
    # available antennas
    if ant_active:
        active_ = [ get_ant_data(ant, opts.receiver_band, dmc_epoch) \
    ...
    if ant_inactive:
        inactive_ = [ get_ant_data(ant, opts.receiver_band, dmc_epoch) for ant in ant_inactive ]
