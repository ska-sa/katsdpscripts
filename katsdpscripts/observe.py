...
def standard_script_options(usage, description):
    ...
    parser.add_option('-u', '--experiment-id',
                      ...
                      'experiment together (UUID generated by default)')
    parser.add_option('--schedule-block-id', type='string',
                      help='Schedule block id code for the observation. This is '
                      'required in order to allocate the correct resources')
...
def verify_and_connect(opts):
    ...
    #    raise ValueError('Please specify the --schedule-block-id option (yes, this is a non-optional option...)')
    ...
    try:
        if opts.schedule_block_id is not None:
            kat = configure_core(sb_id_code = opts.schedule_block_id)
        else:
            ...
            kat = tbuild(system, host_clients = 'all', controlled_clients = 'all')
        user_logger.info("Using KAT connection with configuration=%s sb_id_code=%s\nControlled objects: %s" % (kat.system, opts.schedule_block_id, kat.controlled_objects))
