...
def get_flag_stats(mvf, thisdata=None, flags=None, flags_to_show=None, norm_spec=None):
    ...
    for t in targets:
        ...
        averagespec =  np.nanmean(data[mvf.dumps],axis=0) #/weightsum
        flagfrac = 1. - weightsum/mvf.shape[0].astype(np.float)
        flag_stats[t.name] = {'spectrum': averagespec, 'numrecords_tot': mvf.shape[0], 'flagfrac': flagfrac, 'channel_freqs': mvf.channel_freqs, \
                                'dump_period': mvf.dump_period, 'corr_products': mvf.corr_products}
    mvf.select(reset='T')
    return flag_stats
...
def generate_flag_table(input_file, output_root='.', static_flags=None, 
    ...
    for scan, state, target in mvf.scans():
        ...
        for this_slice in scan_slices:
            ...
            this_data = np.empty((this_slice.stop-this_slice.start,freq_range.stop-freq_range.start,mvf.shape[2],),dtype=np.float32)
            flags = np.zeros((this_slice.stop-this_slice.start,freq_range.stop-freq_range.start,mvf.shape[2],),dtype=np.bool)
...
def generate_rfi_report(input_file,input_flags=None,flags_to_show='all',output_root='.',antenna=None,targets=None,freq_chans=None,do_cross=True,beg_drop=4):
    ...
    if targets is 'all': targets = mvf.catalogue.targets
    ...
    for bline in all_blines:
        ...
        mvf.select(reset='TFB',corrprods=corrprodselect)
        ...
        for target in targets:
            ...
            plot_waterfall_subsample(vis[mvf.dumps[:,np.newaxis],mvf.channels,hh_index],flags[mvf.dumps[:,np.newaxis],mvf.channels,hh_index], \
                                                    mvf.channel_freqs,None,label+'\nHH polarisation',output=pdf)
            plot_waterfall_subsample(vis[mvf.dumps[:,np.newaxis],mvf.channels,vv_index],flags[mvf.dumps[:,np.newaxis],mvf.channels,vv_index], \
