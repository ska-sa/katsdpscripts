...
def read_and_plot_data(filename,output_dir='.',pdf=True,Ku = False,
                        ...
                        write_nd=False,rfi_mask='/var/kat/katsdpscripts/RTS/rfi_mask.pickle',receptor='',**kwargs):
    print 'inside',kwargs
	...
	ant_names=[a.name for a in h5.ants]
	#receptor_name='m053'
        receptor_index=ant_names.index(receptor)
        print(ant_names)
        print('We are now serving customer number: ', receptor_index, ' Receptor: ', ant_names[receptor_index])
        ants = h5.ants[receptor_index:receptor_index+1]   
        print(ants)   
        raw_input("Press Enter to continue...")
